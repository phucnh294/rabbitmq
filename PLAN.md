...
| 7 | `07.rpc/` | Request/reply over RabbitMQ using a correlation ID + reply-to queue | `direct` (default) | ✅ Done |
...
- **07.rpc** — client publishes a request with a `replyTo` queue and `correlationId`, server replies on that queue; demonstrates synchronous-style request/response over async messaging. Implemented as a fibonacci(n) calculator, class-based (`Producer`/`Consumer` wired with a channel + `EventEmitter`) rather than the self-invoking-function style used elsewhere. Gotchas found while building it: RabbitMQ 4.x rejects non-durable non-exclusive queues (`transient_nonexcl_queues` deprecation) — `rpc_queue` must be `{durable: true}`; and `07.rpc/{client,server}/config.js` sit one directory deeper than other folders' configs, so they need `../../.env`, not `../.env`.
